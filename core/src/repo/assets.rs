...
lazy_static! {
    pub static ref ASSETS: AsyncOnce<AssetsRepo> = AsyncOnce::new(AssetsRepo::init());
}
...
impl AssetsRepo {
    async fn init() -> Self {
        Self {
            lock: RwLock::new(get_dirs().data_dir().join("Assets")),
        }
    }
}

#[async_trait]
impl Repo<JavaInfo, JavaBuild> for AssetsRepo {
